			...
			const struct pci_device_id *pci_id)
{
	...
	if (err) {
		printk(KERN_INFO "%s: pci_enable_device failed\n", __func__);
		return err;
	}

	printk(KERN_INFO "%s: aq_pci_func_init\n", __func__);
	err = aq_pci_func_init(pdev);
	if (err) {
		printk(KERN_INFO "%s: aq_pci_func_init failed\n", __func__);
		goto err_pci_func;
	}

	printk(KERN_INFO "%s: aq_ndev_alloc\n", __func__);
	ndev = aq_ndev_alloc();
	if (!ndev) {
		err = -ENOMEM;
		goto err_ndev;
	...
	printk(KERN_INFO "%s: aq_pci_probe_get_hw_by_id\n", __func__);
	...
	printk(KERN_INFO "%s: handling bars\n", __func__);
	for (bar = 0; bar < 4; ++bar) {
		if (IORESOURCE_MEM & pci_resource_flags(pdev, bar)) {
			...
			if (mmio_pa == 0U) {
				printk(KERN_INFO "%s: pci_resource_start failed\n", __func__);
				err = -EIO;
				goto err_free_aq_hw;
			}
			...
			if ((reg_sz <= 24 /*ATL_REGS_SIZE*/)) {
				printk(KERN_INFO "%s: pci_resource_len failed\n", __func__);
				err = -EIO;
				goto err_free_aq_hw;
			}
